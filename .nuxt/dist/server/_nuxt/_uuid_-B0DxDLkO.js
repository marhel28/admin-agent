    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!loading.value && merchant.value) {
        ...
        if (!editing.value) {
          ...
          _push(`</div><h2 class="text-lg font-bold text-slate-800">${ssrInterpolate(merchant.value.store_name || merchant.value.name)}</h2><p class="text-sm text-slate-500 mb-4">${ssrInterpolate(merchant.value.name)}</p><span class="inline-block px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold border border-blue-100 mb-6">${ssrInterpolate(merchant.value.category_store || "Tanpa Kategori")}</span><div class="space-y-3 text-left border-t pt-4"><div class="flex items-center gap-3 text-sm">`);
          _push(ssrRenderComponent(_component_Icon, {
...
//# sourceMappingURL=_uuid_-B0DxDLkO.js.map
